...
if ( ! function_exists( 'deep_posted_by' ) ) :
	...
	function deep_posted_by() {
		printf(
			...
			esc_html__( 'Posted by', 'deep-free' ),
...
if ( ! function_exists( 'deep_comment_count' ) ) :
	...
	function deep_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'deep-free' ), get_the_title() ) );
...
if ( ! function_exists( 'deep_entry_footer' ) ) :
	...
	function deep_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( __( ', ', 'deep-free' ) );
			if ( $categories_list ) {
				printf(
					/* translators: 1: SVG icon. 2: posted in label, only visible to screen readers. 3: list of categories. */
					'<span class="cat-links"><span class="screen-reader-text">%1$s</span>%2$s</span>',
					esc_html__( 'Posted in', 'deep-free' ),
					wp_kses_post($categories_list)
				); 
			}

			/* translators: used between list items, there is a space after the comma. */
			$tags_list = get_the_tag_list( '', __( ', ', 'deep-free' ) );
			if ( $tags_list ) {
				printf(
					/* translators: 1: SVG icon. 2: posted in label, only visible to screen readers. 3: list of tags. */
					'<span class="tags-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
					esc_html__( 'Tags:', 'deep-free' ),				
...
if ( ! function_exists( 'deep_discussion_avatars_list' ) ) :
	...
	function deep_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				esc_html( deep_get_user_avatar_markup( $id_or_email ) )
			);
		}
	...
	function deep_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => sprintf(
					'%s <span class="nav-prev-text main-pg">%s</span>',
					'<',
					__( 'Newer posts', 'deep-free' )
				),
				'next_text' => sprintf(
					'<span class="nav-next-text main-pg">%s</span> %s',
					__( 'Older posts', 'deep-free' ),
					'>'
				),
			)
		);
