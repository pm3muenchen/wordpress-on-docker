...
function deep_get_the_archive_title() {
	if ( is_category() ) {
		$title = __( 'Category Archives: ', 'deep-free' ) . '<span class="page-description">' . single_term_title( '', false ) . '</span>';
	} elseif ( is_tag() ) {
		$title = __( 'Tag Archives: ', 'deep-free' ) . '<span class="page-description">' . single_term_title( '', false ) . '</span>';
	} elseif ( is_author() ) {
		$title = __( 'Author Archives: ', 'deep-free' ) . '<span class="page-description">' . get_the_author_meta( 'display_name' ) . '</span>';
	} elseif ( is_year() ) {
		$title = __( 'Yearly Archives: ', 'deep-free' ) . '<span class="page-description">' . get_the_date( _x( 'Y', 'yearly archives date format', 'deep-free' ) ) . '</span>';
	} elseif ( is_month() ) {
		$title = __( 'Monthly Archives: ', 'deep-free' ) . '<span class="page-description">' . get_the_date( _x( 'F Y', 'monthly archives date format', 'deep-free' ) ) . '</span>';
	} elseif ( is_day() ) {
		$title = __( 'Daily Archives: ', 'deep-free' ) . '<span class="page-description">' . get_the_date() . '</span>';
	} elseif ( is_post_type_archive() ) {
		$title = __( 'Post Type Archives: ', 'deep-free' ) . '<span class="page-description">' . post_type_archive_title( '', false ) . '</span>';
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: %s: Taxonomy singular name */
		$title = sprintf( esc_html__( '%s Archives:', 'deep-free' ), $tax->labels->singular_name );
	} else {
		$title = __( 'Archives:', 'deep-free' );
	}
	return $title;
}
...
function deep_add_dropdown_icons( $output, $item, $depth, $args ) {
	...
	if ( in_array( 'mobile-parent-nav-menu-item', $item->classes, true ) && isset( $item->original_id ) ) {
		...
		$deep_free_link = sprintf(
			'<button class="menu-item-link-return" tabindex="-1">%s',
			esc_html__( 'lorem', 'deep-free' )
		);
