...
$deep_options['deep_blog_index_social_share'] = isset($deep_options['deep_blog_social_share']) ? $deep_options['deep_blog_social_share'] : '1' ;
	...
	<div class="blog-inner">
		...
		<div class="blog-header-metadata">
			...
			<div class="postmetadata">
				...
				if($deep_options['deep_blog_meta_comments_enable']){ ?>
					<h6 class="blog-comments"><a href="<?php echo esc_html( get_comments_link() ); ?>"><i class="ti-comment"></i><?php comments_number(  ); ?></a></h6>
				<?php } ?>
		...
		if( $deep_options['deep_blog_featuredimage_enable'] ) {
			...
			// gallery post type
							...
							<div class="gl-img owl-carousel owl-theme">
								...
								for ($i=0; $i < sizeof($galley_id); $i++) {
									if( !empty( $galley_id[$i] ) ) {
										...
										$thumbnail_url = $image->m_image( $galley_id[$i], wp_get_attachment_url( $galley_id[$i] ) , '1196' , '739' ); // set required and get result
										echo '<img src="' . esc_url($thumbnail_url) . '" alt="' . esc_html( get_the_title() ) . '">';
									}
								} ?>											
							</div>													
						</div>	
				...
				if( has_post_thumbnail() ) {
					if ( !empty($deep_options['deep_image_large_dimensions']['width']) && !empty($deep_options['deep_image_large_dimensions']['height']) ) {
						if ( !class_exists( 'Wn_Img_Maniuplate' ) ) {
							load_template( DEEP_CORE_DIR . 'helper-classes/class_webnus_manuplate.php' );
						}
						$i_width = str_replace("px","",$deep_options['deep_image_large_dimensions']['width']);
						$i_height = str_replace("px","",$deep_options['deep_image_large_dimensions']['height']);
						$image = new Wn_Img_Maniuplate; // instance from settor class
						$thumbnail_url = $image->m_image( get_post_thumbnail_id(), get_the_post_thumbnail_url() , $i_width , $i_height );
						echo '<img src="' . esc_url($thumbnail_url) . '" alt="' . esc_html(get_the_title()) . '">';
					} else {
		...
		<div class="blgt1-inner">
			...
			if( 0 == $deep_options['deep_blog_excerptfull_enable'] ) {
				...
				echo '... <br><br><a class="readmore" href="' . esc_url( get_permalink($post->ID) ) . '">' . esc_html($deep_options['deep_blog_readmore_text']) . '</a>';
				echo '</p>';
				if( 'quote' == $post_format  )
				echo '</blockquote>';
			} else {
