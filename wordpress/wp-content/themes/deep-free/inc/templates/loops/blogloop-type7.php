...
?>
<article id="post-<?php the_ID(); ?>" <?php post_class('blog-post blgtyp1'); ?>>
		...
		if( $deep_options['deep_blog_featuredimage_enable'] ) {
			if( 'video'  == $post_format || 'audio'  == $post_format ) {
				...
				if( has_post_thumbnail() ){
					echo '<a href="' . esc_url( get_the_permalink() ) . '"> '.wp_get_attachment_image( get_post_thumbnail_id(), array( '548', '394' ) ).' </a>';
				} else {
