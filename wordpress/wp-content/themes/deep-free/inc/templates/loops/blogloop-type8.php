			...
			if( 'video'  == $post_format || 'audio'  == $post_format ) {
							...
							<div class="gl-img owl-carousel owl-theme">
								...
								for( $i=0; $i < sizeof( $galley_id ); $i++ ) {
									if( !empty( $galley_id[$i] ) ) {
										...
										$thumbnail_url = $image->m_image( $galley_id[$i], wp_get_attachment_url( $galley_id[$i] ) , '525' , '377'  ); // set required and get result
				...
				if( has_post_thumbnail() ) {
					...
					if( !empty( $thumbnail_id ) ) {
						echo ' <a href="' . esc_url( get_the_permalink() ) . '"> ' . wp_get_attachment_image( $thumbnail_id, array( '548', '394' ) ) . ' </a>';
					}
				} else {
