<?php
/*
 * Elementor Events Addon for Elementor Unique Call To Action Widget
 * Author & Copyright: NicheAddon
*/

namespace Elementor;

if (!isset(get_option( 'eafe_unqw_settings' )['naeafe_unique_call_action'])) { // enable & disable

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Event_Elementor_Addon_Unique_CallToAction extends Widget_Base{

	/**
	 * Retrieve the widget name.
	*/
	public function get_name(){
		return 'naevents_unique_call_action';
	}
	...
	protected function _register_controls(){
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_html__( 'Get Your Call To Action', 'events-addon-for-elementor' ),
	...
	*/
	protected function render() {
		...
		$call_action_title = !empty( $settings['call_action_title'] ) ? $settings['call_action_title'] : '';
	  ...
	  $btn_text = $btn_text ? $btn_text : esc_html__( 'REGISTER NOW', 'events-addon-for-elementor' );
		...
		$link_title = $title_link ? '<a href="'.esc_url($title_link).'" '.$title_link_attr.'>'.esc_html($call_action_title).'</a>' : esc_html($call_action_title);
		$title = $call_action_title ? '<h3>'.$link_title.'</h3>' : '';
		$button = $btn_link ? '<div class="naeep-btn-wrap"><a href="'.esc_url($btn_link).'" class="naeep-btn" '.$btn_link_attr.'>'.esc_html($btn_text).'</a></div>' : '';
        ...
        <a href="javascript:void(0);" class="cta-close"></a>
			  <div class="col-na-row align-items-center">
			    <div class="col-na-9"><?php echo $title; ?></div>
			    <div class="col-na-3"><?php echo $button; ?></div>
			  </div>
			</div>
		<?php
...
Plugin::instance()->widgets_manager->register_widget_type( new Event_Elementor_Addon_Unique_CallToAction() );
