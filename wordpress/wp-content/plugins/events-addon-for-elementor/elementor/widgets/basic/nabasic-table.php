<?php
/*
 * Elementor Events Addon for Elementor Table Widget
 * Author & Copyright: NicheAddon
*/

namespace Elementor;

if (!isset(get_option( 'eafe_bw_settings' )['naeafe_table'])) { // enable & disable

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Event_Elementor_Addon_Table extends Widget_Base{

	/**
	 * Retrieve the widget name.
	*/
	public function get_name(){
		return 'naevents_basic_table';
	}
	...
	*/
	public function get_title(){
		return esc_html__( 'Table', 'events-addon-for-elementor' );
	}
	...
	protected function _register_controls(){
		...
		$this->add_responsive_control(
			...
			[
				...
				'default' => 'center',
				'selectors' => [
					'{{WRAPPER}} .naeep-table td, {{WRAPPER}} .naeep-table thead th' => 'text-align: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => [
					[
						'table_title' => esc_html__( 'Members', 'events-addon-for-elementor' ),
					],
		...
		$this->start_controls_section(
			'section_table_bdy',
			[
				'label' => esc_html__( 'Table Body', 'events-addon-for-elementor' ),
			]
		);
		...
		$repeaterOne->start_controls_tabs( 'table_rows' );
			$repeaterOne->start_controls_tab(
				'table_row1',
				[
					'label' => esc_html__( 'Row', 'events-addon-for-elementor' ),
				]
			);
		...
		$this->add_control(
			'tableItems_row',
			[
				'label' => esc_html__( 'Table Row', 'events-addon-for-elementor' ),
				'type' => Controls_Manager::REPEATER,
				'default' => [
					[
						'row_text1' => esc_html__( 'Item #1', 'events-addon-for-elementor' ),
					],
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} table.naeep-table' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} table.naeep-table tbody>tr:nth-child(odd)>td' => 'background-color: {{VALUE}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} table.naeep-table tbody>tr:nth-child(odd)>td' => 'color: {{VALUE}};',
				],
			]
		);
		$this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} table.naeep-table tbody>tr:nth-child(even)>td' => 'background-color: {{VALUE}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} table.naeep-table tbody>tr:nth-child(even)>td' => 'color: {{VALUE}};',
				],
			]
		);
		$this->end_controls_section();// end: Section
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .naeep-table thead tr' => 'background-color: {{VALUE}};',
				],
			]
		);
		$this->add_group_control(
			...
			[
				'name' => 'secn_border',
				'label' => esc_html__( 'Border', 'events-addon-for-elementor' ),
				'selector' => '{{WRAPPER}} .naeep-table thead tr, {{WRAPPER}} table.naeep-table thead:first-child tr:first-child th',
			]
		);
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .naeep-table thead tr',
			]
		);
		$this->add_group_control(
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'link_options',
				[
					'label' => __( 'Link', 'events-addon-for-elementor' ),
					'type' => Controls_Manager::HEADING,
					'frontend_available' => true,
					'separator' => 'before',
				]
			...
			$this->add_control(
				...
				[
					...
					'type' => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .naeep-table td a:hover' => 'color: {{VALUE}};',
					],
				]
			);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px' ],
				'selectors' => [
					'{{WRAPPER}} .naeep-table td i' => 'font-size: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .naeep-table td i' => 'color: {{VALUE}};',
				],
			]
		);
		$this->end_controls_section();// end: Section
	...
	*/
	protected function render() {
	  ...
	  $output = '<div class="naeep-responsive-table">
						      ...
						      <tbody>';
						      	// Group Param Output
										if ( is_array( $tableItems_row ) && !empty( $tableItems_row ) ){
  										...
  										$button1 = !empty( $text_link1 ) ? '<a href="'.esc_url($text_link1).'" '.$text_link1_attr.' class="naeep-btn">'.esc_html($row_text1).'</a>' : '';
  										...
  										$button2 = !empty( $text_link2 ) ? '<a href="'.esc_url($text_link2).'" '.$text_link2_attr.' class="naeep-btn">'.esc_html($row_text2).'</a>' : '';
  										...
  										$button3 = !empty( $text_link3 ) ? '<a href="'.esc_url($text_link3).'" '.$text_link3_attr.' class="naeep-btn">'.esc_html($row_text3).'</a>' : '';
  										...
  										$button4 = !empty( $text_link4 ) ? '<a href="'.esc_url($text_link4).'" '.$text_link4_attr.' class="naeep-btn">'.esc_html($row_text4).'</a>' : '';
  										...
  										$button5 = !empty( $text_link5 ) ? '<a href="'.esc_url($text_link5).'" '.$text_link5_attr.' class="naeep-btn">'.esc_html($row_text5).'</a>' : '';
...
Plugin::instance()->widgets_manager->register_widget_type( new Event_Elementor_Addon_Table() );
