	...
	$(document).ready(function(){
        ...
        **/
        $( '.pagination-item' ).click( function( event ) {
            event.preventDefault();
            var request_page = $( this ).attr( 'data-request_page' );
            if( $( this ).is( '.current' ) ) {
                return;
            }
            $( '.loading-overlay.theme-loader' ).addClass( 'show' );
            $( '.pagination-item' ).removeClass( 'current' );
            $( this ).addClass( 'current' );

            var page_id = 'theme-browser-page-'+request_page;
            if( $( '#'+page_id ).length > 0 ) {
                $( '.theme-browser-page' ).hide();
                $( '#'+page_id ).show();
                $( '.loading-overlay.theme-loader' ).removeClass( 'show' );
                onecomUpdateURL( request_page );
                return;
            }

            var data = {
                action  : 'onecom_get_more_themes',
                page    : request_page
            }
            $.post( ajaxurl, data, function( response ) {
                $( '.theme-browser' ).append( response );
                $( '.theme-browser-page' ).hide();
                $( '#'+page_id ).show();
                //$( '.theme-browser' ).attr( 'data-current_page', request_page );
                $( '.loading-overlay.theme-loader' ).removeClass( 'show' );
                onecomUpdateURL( request_page );
            });
        } );
