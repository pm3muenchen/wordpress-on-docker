...
if( ! function_exists( 'onecom_themes_listing_config' ) ) {
	function onecom_themes_listing_config( $request = null ) {
		$config = array();
...
if( ! function_exists( 'onecom_fetch_themes' ) ) {
	function onecom_fetch_themes( $page = 1 ) {
		$themes = $new_themes = array();
		...
		if( is_wp_error( $response ) ) {
			...
			if( wp_remote_retrieve_response_code( $response ) != 200 ) {
				...
				if( ! empty($body) && $body->success ) {
					...
					$new_themes = $themes[ $body->data->current_page ];
...
if( ! function_exists( 'onecom_get_more_themes_callback' ) ) {
	function onecom_get_more_themes_callback() {
		...
		global $theme_data;
		$theme_data = onecom_fetch_themes( $page );
